...
**核心代码** ([DefaultMQProducerImpl.java:604](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/producer/DefaultMQProducerImpl.java#L604)):
...
**核心特点** ([TopicPublishInfo.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/producer/TopicPublishInfo.java)):
...
`TopicPublishInfo` 是 Producer 端用于**管理 Topic 发布路由信息**的核心数据结构。它封装了从 NameServer 获取的原始路由数据（`TopicRouteData`），转换为 Producer 可直接使用的消息队列列表 ([TopicPublishInfo.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/producer/TopicPublishInfo.java))。
...
> **TopicRouteData 结构与路由转换机制**：详见 [核心概念篇 - 路由数据模型](./02-核心概念篇.md#44-路由数据模型)。
...
```java
// MQClientAPIImpl.onExceptionImpl() ([MQClientAPIImpl.java:713](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/MQClientAPIImpl.java#L713))
...
// 消息发送时的压缩逻辑 ([DefaultMQProducerImpl.java:964](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/producer/DefaultMQProducerImpl.java#L964))
if (body.length >= this.defaultMQProducer.getCompressMsgBodyOverHowmuch()) {
    byte[] data = compressor.compress(body, compressLevel);  // 压缩级别5
    msg.setBody(data);
    return true;  // 设置 sysFlag 标志位
}
```
...
**Consumer 端解压源码** ([PullAPIWrapper.java:68](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/consumer/PullAPIWrapper.java#L68))：

```java
List<MessageExt> msgList = MessageDecoder.decodesBatch(
    byteBuffer,
    this.mQClientFactory.getClientConfig().isDecodeReadBody(),
    this.mQClientFactory.getClientConfig().isDecodeDecompressBody(),  // 默认 true
    true
);
```

[MessageDecoder.java:483](file:///d:/work/github/rocketmq/common/src/main/java/org/apache/rocketmq/common/message/MessageDecoder.java#L483)：
...
**关键源码** ([MQFaultStrategy.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/latency/MQFaultStrategy.java))：

```java
public void updateFaultItem(final String brokerName, final long currentLatency, boolean isolation) {
    if (this.sendLatencyFaultEnable) {
        // isolation=true 时使用 30s 作为延迟计算隔离时间
...
根据 `computeNotAvailableDuration` 方法从后往前匹配的规则：
...
> **注意**：当 `isolation=true` 时，使用 `30s` 作为延迟值计算隔离时间，对应隔离 10min。
...
producer.setTimeout(5000);                              // 超时时间 5s
...
`SendResult` 封装了消息发送的返回结果，包含发送状态、消息ID、队列信息等 ([SendResult.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/producer/SendResult.java))：
...
**源码分析** ([DefaultMQProducerImpl.java:707-730](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/impl/producer/DefaultMQProducerImpl.java#L707-L730))：
...
info += FAQUrl.suggestDo(FAQUrl.SEND_MSG_FAILED);
...
> 以上配置项定义见 [DefaultMQProducer.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/producer/DefaultMQProducer.java)。
...
> Producer 路由缓存管理负责维护 Topic 路由信息，是消息发送的基础设施。核心概念（TopicRouteData、QueueData、BrokerData）详见 [核心概念篇 - 路由数据模型](./02-核心概念篇.md#44-路由数据模型)。
...
> **TopicRouteData 结构与路由转换机制**：详见 [核心概念篇 - 路由转换与队列映射](./02-核心概念篇.md#443-路由转换与队列映射)。
