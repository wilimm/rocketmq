...
| 组件 | 功能描述 | 部署特性 | 源码入口 |
| --- | --- | --- | --- |
| **Producer** | 消息发布角色，支持分布式集群部署 | 完全无状态，可水平扩展 | [DefaultMQProducer.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/producer/DefaultMQProducer.java) |
| **Consumer** | 消息消费角色，支持分布式集群部署 | 支持 Push/Pull 两种消费模式 | [DefaultMQPushConsumer.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/consumer/DefaultMQPushConsumer.java) |
| **NameServer** | Topic 路由注册中心，类似服务发现 | 几乎无状态，节点间无信息同步 | [NamesrvStartup.java](file:///d:/work/github/rocketmq/namesrv/src/main/java/org/apache/rocketmq/namesrv/NamesrvStartup.java) |
| **BrokerServer** | 消息存储、投递和查询，高可用保证 | 支持 Master-Slave 架构 | [BrokerStartup.java](file:///d:/work/github/rocketmq/broker/src/main/java/org/apache/rocketmq/broker/BrokerStartup.java) |
...
| `broker` | 消息存储与投递 | [BrokerStartup.java](file:///d:/work/github/rocketmq/broker/src/main/java/org/apache/rocketmq/broker/BrokerStartup.java)、[BrokerController.java](file:///d:/work/github/rocketmq/broker/src/main/java/org/apache/rocketmq/broker/BrokerController.java) |
| `client` | 客户端 API | [DefaultMQProducer.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/producer/DefaultMQProducer.java)、[DefaultMQPushConsumer.java](file:///d:/work/github/rocketmq/client/src/main/java/org/apache/rocketmq/client/consumer/DefaultMQPushConsumer.java) |
| `common` | 公共组件 | `MixAll.java`、`TopicConfig.java` |
| `filter` | 消息过滤 | `ExpressionMessageFilter.java` |
| `logging` | 日志组件 | `InternalLogger.java` |
| `namesrv` | 服务发现与路由 | [NamesrvStartup.java](file:///d:/work/github/rocketmq/namesrv/src/main/java/org/apache/rocketmq/namesrv/NamesrvStartup.java)、[RouteInfoManager.java](file:///d:/work/github/rocketmq/namesrv/src/main/java/org/apache/rocketmq/namesrv/routeinfo/RouteInfoManager.java) |
...
| `store` | 消息存储 | [CommitLog.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/CommitLog.java)、[ConsumeQueue.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java)、[MappedFile.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/logfile/MappedFile.java) |
...
| --- | --- | --- | --- | --- |
| **CommitLog** | 默认 1GB | 变长（消息体+元数据） | `TOTALSIZE`(4B) + `MAGICCODE`(4B) + `BODYCRC`(4B) + `QUEUEID`(4B) + `FLAG`(4B) + `QUEUEOFFSET`(8B) + `PHYSICALOFFSET`(8B) + `SYSFLAG`(4B) + `BORNTIMESTAMP`(8B) + `BORNHOST`(8B/20B) + `STORETIMESTAMP`(8B) + `STOREHOST`(8B/20B) + `RECONSUMETIMES`(4B) + `TRANSACTIONOFFSET`(8B) + `BODY` + `TOPIC` + `PROPERTIES` | [CommitLog.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/CommitLog.java) |
| **ConsumeQueue** | 默认约 5.7MB（300000 * 20 字节） | 固定 20 字节 | `commitlogOffset`(8B) + `msgSize`(4B) + `tagsCode`(8B) | [ConsumeQueue.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java) |
| **IndexFile** | 约 40MB | 索引条目 20 字节 | `keyHash`(4B) + `phyOffset`(8B) + `timeDiff`(4B) + `slotValue`(4B) | [IndexFile.java](file:///d:/work/github/rocketmq/store/src/main/java/org/apache/rocketmq/store/index/IndexFile.java) |
