...
**核心源码**：[ProxyStartup.java](../../proxy/src/main/java/org/apache/rocketmq/proxy/ProxyStartup.java) - Proxy 启动入口
...
```properties
# proxy.properties
proxyMode=CLUSTER                                    # 部署模式，默认 CLUSTER
namesrvAddr=localhost:9876                           # NameServer 地址
grpcServerPort=8081                                  # gRPC 服务端口，默认 8081
grpcThreadPoolNums=16 + CPU核心数*2                   # gRPC 线程池大小，默认 16 + CPU核心数*2
grpcThreadPoolQueueCapacity=100000                   # gRPC 线程池队列容量，默认 100000
grpcMaxInboundMessageSize=136314880                  # gRPC 最大入站消息大小，默认 130MB
maxMessageSize=4194304                               # 消息体最大大小，默认 4MB
maxUserPropertySize=16384                            # 用户属性最大大小，默认 16KB
userPropertyMaxNum=128                               # 用户属性最大数量，默认 128
maxMessageGroupSize=64                               # 消息组最大大小，默认 64
minInvisibleTimeMillsForRecv=10000                   # Pop 消费最小不可见时间，默认 10s
maxInvisibleTimeMills=43200000                       # Pop 消费最大不可见时间，默认 12h
enableACL=false                                      # 是否启用 ACL，默认 false
```
...
```properties
# broker.conf
enableProxyInBroker=true                             # 在 Broker 中启用 Proxy
grpcServerPort=8081                                  # gRPC 服务端口
```

> 源码位置：[ProxyConfig.java](../../proxy/src/main/java/org/apache/rocketmq/proxy/config/ProxyConfig.java)
...
| 组件 | 说明 |
| --- | --- |
| **DLedgerController** | 基于 Raft 的控制器，管理 Broker 元数据 ([DLedgerController.java](../../controller/src/main/java/org/apache/rocketmq/controller/impl/DLedgerController.java)) |
| **SyncStateSet** | 同步副本集合，选主的重要依据 ([SyncStateSet.java](../../common/src/main/java/org/apache/rocketmq/common/protocol/body/SyncStateSet.java)) |
| **AutoSwitchHAService** | 支持 BrokerRole 切换的 HA 服务 ([AutoSwitchHAService.java](../../store/src/main/java/org/apache/rocketmq/store/ha/autoswitch/AutoSwitchHAService.java)) |
| **ReplicasManager** | 副本管理器，监控 HA 状态并同步控制指令 ([ReplicasManager.java](../../broker/src/main/java/org/apache/rocketmq/broker/controller/ReplicasManager.java)) |
...
```properties
# namesrv.conf
enableControllerInNameServer=true                     # 在 NameServer 中启用 Controller
controllerDLedgerGroup=controller-group               # DLedger 组名
controllerDLedgerPeers=n0@127.0.0.1:9878;n1@127.0.0.1:9879;n2@127.0.0.1:9880  # 集群节点
controllerDLedgerSelfId=n0                            # 当前节点 ID
scanNotActiveBrokerInterval=5000                      # 扫描非活跃 Broker 间隔，默认 5s
controllerThreadPoolNums=16                           # 处理线程数，默认 16
controllerRequestThreadPoolQueueCapacity=50000        # 请求线程池队列容量，默认 50000
enableElectUncleanMaster=false                        # 是否允许选举非同步副本，默认 false
notifyBrokerRoleChanged=true                          # 角色变更时是否通知 Broker，默认 true
mappedFileSize=1073741824                             # 元数据映射文件大小，默认 1GB
controllerStorePath=${user.home}/DledgerController    # Controller 存储路径
```
...
> 源码位置：[ControllerConfig.java](../../common/src/main/java/org/apache/rocketmq/common/ControllerConfig.java)

**Broker 端启用 Controller 模式配置** ([BrokerConfig.java](../../common/src/main/java/org/apache/rocketmq/common/BrokerConfig.java))：

```properties
# broker.conf
enableControllerMode=true                             # 启用 Controller 模式
controllerAddr=controller-0:9878;controller-1:9878;controller-2:9878  # Controller 集群地址
syncBrokerMetadataPeriod=5000                         # 同步 Broker 元数据周期，默认 5s
checkSyncStateSetPeriod=5000                          # 检查 SyncStateSet 周期，默认 5s
syncControllerMetadataPeriod=10000                    # 同步 Controller 元数据周期，默认 10s
```
...
RocketMQ 提供两种自动故障转移方案，均基于 Raft 协议实现：

#### 4.5.1 DLedger 模式架构

DLedger 模式是 RocketMQ 4.5+ 引入的自动故障转移方案，将 CommitLog 存储与 Raft 共识协议深度集成：
...
dLegerPeers=n0@127.0.0.1:10912;n1@127.0.0.1:10913;n2@127.0.0.1:10914
dLegerSelfId=n0
```

**核心源码**：[DLedgerCommitLog.java](../../store/src/main/java/org/apache/rocketmq/store/dledger/DLedgerCommitLog.java)
...
| 场景 | 推荐方案 |
| --- | --- |
| 新项目（5.0+） | Controller 模式 |
| 存量项目升级（4.x） | DLedger 模式 |
| 最小化部署成本 | Controller 模式（2 副本） |
| 强一致性要求 | DLedger 模式 |
...
| 组件 | 说明 |
| --- | --- |
| **PopMessageProcessor** | Pop 消息处理器，处理 Pop 请求 ([PopMessageProcessor.java](../../broker/src/main/java/org/apache/rocketmq/broker/processor/PopMessageProcessor.java)) |
| **PopBufferMergeService** | Pop 缓冲合并服务，管理 Pop 消息的 ACK 状态 ([PopBufferMergeService.java](../../broker/src/main/java/org/apache/rocketmq/broker/processor/PopBufferMergeService.java)) |
| **PopCheckPoint** | Pop 消费检查点，记录 Pop 消息的元数据 ([PopCheckPoint.java](../../store/src/main/java/org/apache/rocketmq/store/pop/PopCheckPoint.java)) |
| **AckMsg** | Pop 消息确认结构，用于 ACK 请求 ([AckMsg.java](../../store/src/main/java/org/apache/rocketmq/store/pop/AckMsg.java)) |
| **PopResult** | Pop 消费返回结果 ([PopResult.java](../../client/src/main/java/org/apache/rocketmq/client/consumer/PopResult.java)) |
| **PopStatus** | Pop 消费状态枚举 ([PopStatus.java](../../client/src/main/java/org/apache/rocketmq/client/consumer/PopStatus.java)) |
...
`PopResult` 是 Pop 消费模式的返回结果 ([PopResult.java](../../client/src/main/java/org/apache/rocketmq/client/consumer/PopResult.java))：
...
**客户端配置** ([DefaultMQPushConsumer.java](../../client/src/main/java/org/apache/rocketmq/client/consumer/DefaultMQPushConsumer.java))：

| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `popInvisibleTime` | 60000 (60s) | 消息不可见时间，范围 [5000, 300000] (ms) |
| `popBatchNums` | 32 | 单次 Pop 最大消息数，范围 [1, 32] |
| `clientRebalance` | true | 是否客户端 Rebalance (Pop 需设为 false) |

**Broker 端配置** ([BrokerConfig.java](../../common/src/main/java/org/apache/rocketmq/common/BrokerConfig.java))：

| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `popPollingSize` | 1024 | Pop 长轮询队列大小 |
| `popPollingMapSize` | 100000 | Pop 轮询 Map 大小 |
| `maxPopPollingSize` | 100000 | 最大 Pop 轮询大小 |
| `reviveQueueNum` | 8 | Revive 队列数量 |
| `reviveInterval` | 1000 (1s) | Revive 扫描间隔 |
| `popCkStayBufferTime` | 10000 (10s) | Pop CheckPoint 缓冲时间 |
| `popCkStayBufferTimeOut` | 3000 (3s) | Pop CheckPoint 缓冲超时 |
| `popCkMaxBufferSize` | 200000 | Pop CheckPoint 最大缓冲大小 |
| `enablePopBufferMerge` | false | 是否启用 Pop 缓冲合并 |
