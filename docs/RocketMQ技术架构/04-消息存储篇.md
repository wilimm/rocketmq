...
CommitLog 是消息存储的核心组件，负责消息的顺序写入和持久化存储，是 RocketMQ 高吞吐量的基础 ([CommitLog.java](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java))。
...
RocketMQ 支持批量消息写入，通过 `putMessages()` 方法将多条消息一次性写入 CommitLog ([CommitLog.java:935-1075](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java#L935-L1075))。
...
消息写入 CommitLog 后返回 `PutMessageResult`，其中包含 `PutMessageStatus` 状态码，表示写入结果。

#### 2.3.1 AppendMessageStatus 枚举

`AppendMessageStatus` 表示消息追加到 MappedFile 的结果 ([AppendMessageStatus.java](../../store/src/main/java/org/apache/rocketmq/store/AppendMessageStatus.java))：

| 状态值 | 说明 |
|--------|------|
| `PUT_OK` | 写入成功 |
| `END_OF_FILE` | 当前文件已满，需要创建新文件重新写入 |
| `MESSAGE_SIZE_EXCEEDED` | 消息体超过最大限制（默认 4MB） |
| `PROPERTIES_SIZE_EXCEEDED` | 消息属性超过最大限制（默认 32KB） |
| `UNKNOWN_ERROR` | 未知错误 |

#### 2.3.2 PutMessageStatus 枚举

`PutMessageStatus` 表示消息写入的最终结果，包含刷盘和主从同步状态 ([PutMessageStatus.java](../../store/src/main/java/org/apache/rocketmq/store/PutMessageStatus.java))：

| 状态值 | 说明 |
|--------|------|
| `PUT_OK` | 写入成功 |
| `FLUSH_DISK_TIMEOUT` | 刷盘超时（同步刷盘模式下） |
| `FLUSH_SLAVE_TIMEOUT` | 主从同步超时（同步复制模式下） |
| `SLAVE_NOT_AVAILABLE` | 从节点不可用 |
| `SERVICE_NOT_AVAILABLE` | 服务不可用 |
| `CREATE_MAPPED_FILE_FAILED` | 创建映射文件失败 |
| `MESSAGE_ILLEGAL` | 消息非法 |
| `PROPERTIES_SIZE_EXCEEDED` | 属性大小超限 |
| `OS_PAGE_CACHE_BUSY` | OS PageCache 繁忙 |
| `UNKNOWN_ERROR` | 未知错误 |
| `IN_SYNC_REPLICAS_NOT_ENOUGH` | 同步副本数不足 |
| `PUT_TO_REMOTE_BROKER_FAIL` | 写入远程 Broker 失败 |
| `LMQ_CONSUME_QUEUE_NUM_EXCEEDED` | LMQ 消费队列数量超限 |
| `WHEEL_TIMER_FLOW_CONTROL` | 时间轮流量控制 |
| `WHEEL_TIMER_MSG_ILLEGAL` | 延时消息非法 |
| `WHEEL_TIMER_NOT_ENABLE` | 延时消息功能未启用 |
...
| 类名 | 源码位置 | 职责 |
|------|----------|------|
| [CommitLog](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java) | CommitLog.java | 消息存储入口，管理消息写入和读取 |
| [MappedFileQueue](../../store/src/main/java/org/apache/rocketmq/store/MappedFileQueue.java) | MappedFileQueue.java | 管理多个 MappedFile 的队列 |
| [MappedFile](../../store/src/main/java/org/apache/rocketmq/store/logfile/MappedFile.java) | MappedFile.java | 单个物理文件的接口定义 |
| [DefaultMappedFile](../../store/src/main/java/org/apache/rocketmq/store/logfile/DefaultMappedFile.java) | DefaultMappedFile.java | MappedFile 的默认实现 |
...
源码依据 - [MessageStoreConfig.java:48](../../store/src/main/java/org/apache/rocketmq/store/config/MessageStoreConfig.java#L48)：
```java
private int mappedFileSizeCommitLog = 1024 * 1024 * 1024;  // 1GB
```

源码依据 - [MappedFileQueue.java:207-211](../../store/src/main/java/org/apache/rocketmq/store/MappedFileQueue.java#L207-L211)：
```java
...
源码位置 - [DefaultMappedFile.java:87-88](../../store/src/main/java/org/apache/rocketmq/store/logfile/DefaultMappedFile.java#L87-L88)：
...
**核心字段** - [DefaultMappedFile.java:54-76](../../store/src/main/java/org/apache/rocketmq/store/logfile/DefaultMappedFile.java#L54-L76)：
...
**双重 slice 源码** - [DefaultMappedFile.java:419-424](../../store/src/main/java/org/apache/rocketmq/store/logfile/DefaultMappedFile.java#L419-L424)：
...
#### 2.6.2 SelectMappedBufferResult 详解

`SelectMappedBufferResult` 是消息读取的**核心数据结构**，封装了从 MappedFile 中查询到的消息数据 ([SelectMappedBufferResult.java](../../store/src/main/java/org/apache/rocketmq/store/SelectMappedBufferResult.java))：
...
ConsumeQueue 是按 Topic-QueueId 组织的消息索引文件，用于快速定位消息在 CommitLog 中的位置，每条记录固定 20 字节 ([ConsumeQueue.java](../../store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java))。
...
| 特性 | 说明 |
|------|------|
| **Tag 数量限制** | 每条消息**只能设置一个 Tag**，不支持多个 Tag |
| **存储方式** | 存储单个 Tag 字符串的 `hashCode()` 值 ([MessageExtBrokerInner.java:38-42](../../common/src/main/java/org/apache/rocketmq/common/message/MessageExtBrokerInner.java#L38-L42)) |
| **过滤机制** | Broker 端通过 Hash 值比对实现快速过滤 |
| **空 Tag 处理** | 未设置 Tag 时，Tags HashCode 值为 0 |

**多标签场景替代方案**：

| 方案 | 说明 | 示例 |
|------|------|------|
| **SQL92 过滤** | 使用 `putUserProperty()` 设置多个自定义属性 | `message.putUserProperty("region", "beijing")` |
| **Keys** | 使用 `setKeys()` 设置多个 Key（空格分隔），用于查询 | `message.setKeys("key1 key2 key3")` |
| **多 Topic** | 将消息发送到多个 Topic，每个 Topic 代表一个分类 | - |
...
**常量定义** - [ConsumeQueue.java:30](../../store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java#L30)：
...
**核心实现** - [ConsumeQueue.java:532-551](../../store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java#L532-L551)：
...
**核心源码** ([MappedFileQueue.java](../../store/src/main/java/org/apache/rocketmq/store/MappedFileQueue.java))：
...
**CommitLog 读取消息** ([CommitLog.java](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java))：
...
### 3.5 ConsumeQueue 扩展类型

RocketMQ 提供了两种 ConsumeQueue 实现，满足不同场景需求：

| 类型 | 类名 | 单元大小 | 适用场景 |
|------|------|----------|----------|
| **SimpleCQ** | [ConsumeQueue.java](../../store/src/main/java/org/apache/rocketmq/store/ConsumeQueue.java) | 20 字节 | 普通消息，默认类型 |
| **BatchCQ** | [BatchConsumeQueue.java](../../store/src/main/java/org/apache/rocketmq/store/queue/BatchConsumeQueue.java) | 46 字节 | 批量消息优化 |

**BatchConsumeQueue 单元结构** - [BatchConsumeQueue.java:42-49](../../store/src/main/java/org/apache/rocketmq/store/queue/BatchConsumeQueue.java#L42-L49)：
...
| 字段 | 大小 | 说明 |
|------|------|------|
| position | 8B | CommitLog 物理偏移量 |
| size | 4B | 消息大小 |
| tagsCode | 8B | Tag 哈希值 |
| storeTime | 8B | 消息存储时间 |
| msgBaseOffset | 8B | 批量消息基础偏移量 |
| batchSize | 2B | 批量消息数量 |
...
IndexFile 是按消息 Key 组织的哈希索引文件，支持通过消息 Key 快速查询消息 ([IndexFile.java](../../store/src/main/java/org/apache/rocketmq/store/index/IndexFile.java))。
...
IndexHeader 占用 40 字节，记录文件的元数据信息 ([IndexHeader.java](../../store/src/main/java/org/apache/rocketmq/store/index/IndexHeader.java))：
...
**文件大小计算** - [IndexFile.java:42-44](../../store/src/main/java/org/apache/rocketmq/store/index/IndexFile.java#L42-L44)：
...
默认配置下：
```
fileTotalSize = 40 + 5,000,000 × 4 + 20,000,000 × 20
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `maxHashSlotNum` | 5,000,000 (500万) | 哈希槽数量 |
| `maxIndexNum` | 20,000,000 (2000万，即 maxHashSlotNum × 4) | 索引条目数量 |

### 4.4 索引写入流程

索引写入由 `CommitLogDispatcherBuildIndex` 触发，调用 `IndexFile.putKey` 方法 ([IndexFile.java:92-158](../../store/src/main/java/org/apache/rocketmq/store/index/IndexFile.java#L92-L158))：
...
public boolean putKey(final String key, final long phyOffset, final long storeTimestamp) {
    if (this.indexHeader.getIndexCount() < this.indexNum) {
        ...
        this.mappedByteBuffer.putInt(absIndexPos, keyHash);            // Key哈希 (4B)
        this.mappedByteBuffer.putLong(absIndexPos + 4, phyOffset);     // 物理偏移 (8B)
        this.mappedByteBuffer.putInt(absIndexPos + 12, (int)timeDiff); // 时间差 (4B)
        this.mappedByteBuffer.putInt(absIndexPos + 16, slotValue);     // 前一个索引位置 (4B)
...
通过 Key 查询消息时，调用 `IndexFile.selectPhyOffset` 方法 ([IndexFile.java:188-230](../../store/src/main/java/org/apache/rocketmq/store/index/IndexFile.java#L188-L230))：
...
IndexFile 的写入由 `CommitLogDispatcherBuildIndex` 触发，在消息分发时异步构建索引 ([DefaultMessageStore.java:1878-1886](../../store/src/main/java/org/apache/rocketmq/store/DefaultMessageStore.java#L1878-L1886))。

#### 4.6.1 写入条件判断流程

```mermaid
flowchart TD
    A[消息分发到 IndexService] --> B{messageIndexEnable?}
    B -->|否| C[跳过索引构建]
    B -->|是| D{事务类型判断}
    
    D -->|TRANSACTION_ROLLBACK| E[跳过，不写入索引]
    D -->|其他类型| F{消息有 Key?}
    
    F -->|uniqKey 或 keys 为空| G[不写入任何索引条目]
    F -->|有 uniqKey| H[写入 uniqKey 索引]
    F -->|有 keys| I[遍历写入每个 key 索引]
...
#### 4.6.2 会写入 IndexFile 的情况

| 条件 | 说明 |
|------|------|
| **全局开关开启** | `messageIndexEnable = true`（默认开启） |
| **消息有 uniqKey** | 由客户端自动生成的消息唯一标识，几乎所有消息都有 |
| **消息有 keys** | 用户通过 `setKeys()` 设置的业务 Key |
| **事务非回滚** | 普通消息、半消息、已提交事务消息都会写入 |

```java
// IndexService.java:buildIndex()
public void buildIndex(DispatchRequest req) {
    ...
    if (indexFile != null) {
        ...
        switch (tranType) {
            ...
            case MessageSysFlag.TRANSACTION_COMMIT_TYPE:     // 已提交   ✓
                break;
            case MessageSysFlag.TRANSACTION_ROLLBACK_TYPE:   // 已回滚   ✗
                return;
        }
...
#### 4.6.3 不会写入 IndexFile 的情况

| 情况 | 原因 |
|------|------|
| **`messageIndexEnable = false`** | 全局开关关闭，不构建任何索引 |
| **事务回滚消息** | `TRANSACTION_ROLLBACK_TYPE` 直接 return |
| **消息无 uniqKey 且无 keys** | 两者都为空时，不会写入任何索引条目 |
| **重复消息** | `commitLogOffset < endPhyOffset` 时跳过 |
| **IndexFile 创建失败** | `retryGetAndCreateIndexFile()` 返回 null |

#### 4.6.4 不同消息类型的索引写入

| 消息类型 | 是否写入 IndexFile | 说明 |
|----------|-------------------|------|
| 普通消息（有 uniqKey） | ✅ 写入 | 绝大多数情况 |
| 普通消息（无 uniqKey，无 keys） | ❌ 不写入 | 极少数手动构造的情况 |
| 事务半消息 | ✅ 写入 | PREPARED 状态 |
| 事务提交消息 | ✅ 写入 | COMMIT 状态 |
| 事务回滚消息 | ❌ 不写入 | ROLLBACK 状态 |

**实际场景**：由于 `uniqKey` 由客户端自动生成（属性 `UNIQ_KEY`），**绝大多数消息都会写入 IndexFile**。只有显式关闭索引功能、事务回滚、或极少数不设置任何 Key 的消息才不会写入。
...
同步刷盘通过 `GroupCommitService` 实现，写入后立即刷盘并等待完成 ([CommitLog.java:1446](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java#L1446))。
...
异步刷盘通过 `FlushRealTimeService` 实现，后台线程定期执行刷盘操作 ([CommitLog.java:1319](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java#L1319))。
...
同步刷盘模式下，`FlushDiskWatcher` 监控刷盘请求的超时状态 ([FlushDiskWatcher.java](../../store/src/main/java/org/apache/rocketmq/store/FlushDiskWatcher.java))：
...
public void run() {
    while (!isStopped()) {
        ...
        while (!request.future().isDone()) {
            ...
            // 避免频繁线程切换，使用 sleep 替代 future.get
...
**工作机制**：
- 每个同步刷盘请求创建时设置超时截止时间（`syncFlushTimeout`）
- `FlushDiskWatcher` 独立线程监控请求是否超时
- 超时后设置 `FLUSH_DISK_TIMEOUT` 状态，避免无限等待
...
### 7.2 StoreCheckpoint 检查点文件

`StoreCheckpoint` 是 Broker 的存储检查点文件，记录各存储组件的最新刷盘时间戳，用于异常恢复时确定数据一致性边界 ([StoreCheckpoint.java](../../store/src/main/java/org/apache/rocketmq/store/StoreCheckpoint.java))。

#### 7.2.1 检查点字段说明

| 字段 | 大小 | 说明 |
|------|------|------|
| `physicMsgTimestamp` | 8B | CommitLog 最新刷盘时间戳 |
| `logicsMsgTimestamp` | 8B | ConsumeQueue 最新刷盘时间戳 |
| `indexMsgTimestamp` | 8B | IndexFile 最新刷盘时间戳 |
| `masterFlushedOffset` | 8B | Master 已刷盘偏移量（主从同步用） |

**源码定义** - [StoreCheckpoint.java:32-35](../../store/src/main/java/org/apache/rocketmq/store/StoreCheckpoint.java#L32-L35)：

```java
private volatile long physicMsgTimestamp = 0;   // CommitLog 刷盘时间
private volatile long logicsMsgTimestamp = 0;   // ConsumeQueue 刷盘时间
private volatile long indexMsgTimestamp = 0;    // IndexFile 刷盘时间
private volatile long masterFlushedOffset = 0;  // Master 刷盘偏移量
```
...
| 场景 | 作用 |
|------|------|
| **异常恢复** | 根据 `minTimestamp` 确定恢复起点，避免恢复不完整数据 |
| **数据一致性** | 确保三个存储组件（CommitLog、ConsumeQueue、IndexFile）的时间戳一致性 |
| **主从同步** | `masterFlushedOffset` 用于 Slave 确定同步起点 |

**恢复时的时间戳判断** - [StoreCheckpoint.java](../../store/src/main/java/org/apache/rocketmq/store/StoreCheckpoint.java)：

```java
// 获取最小时间戳，用于异常恢复
public long getMinTimestamp() {
    return Math.min(this.physicMsgTimestamp, Math.min(this.logicsMsgTimestamp, this.indexMsgTimestamp));
}

// 获取最小时间戳（带索引安全检查）
public long getMinTimestampIndex() {
    if (messageStoreConfig.isMessageIndexSafe()) {
...
| 组件 | 更新时机 | 更新方法 |
|------|----------|----------|
| **CommitLog** | 消息刷盘后 | `setPhysicMsgTimestamp()` |
| **ConsumeQueue** | 索引构建刷盘后 | `setLogicsMsgTimestamp()` |
| **IndexFile** | 索引刷盘后 | `setIndexMsgTimestamp()` |
...
`CommitLog.recoverNormally` 实现正常恢复逻辑 ([CommitLog.java:287](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java#L287))。
...
`CommitLog.recoverAbnormally` 实现异常恢复逻辑，从最后一个文件向前查找有效文件开始恢复 ([CommitLog.java:595](../../store/src/main/java/org/apache/rocketmq/store/CommitLog.java#L595))。
...
| `maxIndexNum` | 20000000 (2000万，即 maxHashSlotNum × 4) | IndexFile 索引条目数量 |
...
| 配置项 | 计算公式 | 说明 |
|--------|----------|------|
| `mappedFileSizeCommitLog` | 1024 × 1024 × 1024 = 1,073,741,824 | 1GB |
| `mappedFileSizeConsumeQueue` | 300,000 × 20 = 6,000,000 | 约 5.7MB，可存储 30 万条索引 |
...
1. **二层存储架构**：CommitLog（顺序写入）+ ConsumeQueue（消费索引），IndexFile 是可选的扩展查询索引
