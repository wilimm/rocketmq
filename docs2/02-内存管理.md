...
| 层次 | 核心技术 | 关键组件 | 优化目标 |
|------|---------|---------|---------|
| 虚拟内存层 | 虚拟地址空间、页表映射、TLB缓存 | task_struct、mm_struct | 进程隔离、内存扩展 |
| 内存映射层 | mmap文件映射、1GB映射单元 | MappedFile、位置指针 | 高效文件访问 |
| 零拷贝层 | sendfile、transferTo、DMA | FileChannel | 减少数据拷贝 |
| 堆外内存层 | DirectByteBuffer、内存池化 | TransientStorePool | 避免GC停顿 |
| 内存优化层 | mlock锁定、madvise建议、预热 | UtilAll | 保障访问性能 |
| 资源管理层 | 引用计数、内存交换 | ReferenceResource | 安全释放资源 |
...
**虚拟内存**为每个进程提供独立的地址空间，通过页表映射到物理内存。这是现代操作系统最核心的内存管理机制，实现了进程隔离、内存扩展和共享内存等功能。

**设计思想**：
- 每个进程拥有独立的虚拟地址空间，互不干扰
- 虚拟地址通过页表映射到物理地址，由MMU硬件加速
- 支持按需分页(Demand Paging)，页面按需加载
- 支持内存超额分配(Overcommit)和写时复制(COW)

#### 内核数据结构

```c
// 进程内存描述符（include/linux/mm_types.h）
struct mm_struct {
    ...
    unsigned long locked_vm;        // 被mlock锁定的页数
    unsigned long pinned_vm;        // 被pin的页数
    ...
    struct rw_semaphore mmap_sem;   // VMA读写信号量
};

// 虚拟内存区域（include/linux/mm_types.h）
struct vm_area_struct {
    ...
    unsigned long vm_flags;         // 标志位：VM_READ, VM_WRITE, VM_EXEC
};
```
...
| 字段 | 类型 | 说明 | 用途 |
|------|------|------|------|
| `pgd` | pgd_t* | 页全局目录指针 | 地址转换的起点 |
| `mmap` | vm_area_struct* | VMA链表头 | 遍历所有内存区域 |
| `mm_rb` | rb_root | VMA红黑树 | 快速查找内存区域 |
| `total_vm` | unsigned long | 总虚拟内存页数 | 内存使用统计 |
| `start_stack` | unsigned long | 栈起始地址 | 线程栈管理 |
...
**页表**是虚拟地址到物理地址的映射表，x86_64架构使用4级页表结构。TLB(Translation Lookaside Buffer)是页表的硬件缓存，缓存最近使用的页表项。

**设计思想**：
- 多级页表减少内存占用，只有使用的页才分配页表项
- TLB缓存加速地址转换，命中时纳秒级完成
- 页表项包含权限位和脏位，支持内存保护和写回优化
- 支持大页(Huge Page)减少页表层级和TLB压力

#### 内核数据结构

```c
// x86_64 四级页表结构
// PGD (Page Global Directory)     - 页全局目录，512项
// PUD (Page Upper Directory)      - 页上级目录，512项
// PMD (Page Middle Directory)     - 页中间目录，512项
// PTE (Page Table Entry)          - 页表项，512项
...
// 页表项标志位（x86_64）
#define _PAGE_PRESENT    0x001     // 页面存在于物理内存
...
#define _PAGE_DIRTY      0x040     // 已修改（脏页）
#define _PAGE_PSE        0x080     // 大页
#define _PAGE_GLOBAL     0x100     // 全局页（TLB不刷新）
#define _PAGE_NX         0x8000000000000000ULL  // 不可执行

// 地址转换宏
#define PGDIR_SHIFT     39         // PGD索引位移
#define PUD_SHIFT       30         // PUD索引位移
#define PMD_SHIFT       21         // PMD索引位移
#define PAGE_SHIFT      12         // 页内偏移位移
#define PTRS_PER_PGD    512        // 每级页表项数
```
...
**缺页中断(Page Fault)**是当进程访问的虚拟页面不在物理内存中时触发的异常，由内核的缺页处理程序处理。缺页中断是实现按需分页(Demand Paging)的核心机制。

**缺页类型**：
- **硬缺页(Hard Page Fault)**：页面需要从磁盘读取
- **软缺页(Soft Page Fault)**：页面在内存中但未映射（如共享内存）
- **次要缺页(Minor Fault)**：页面在Page Cache中
...
| 参数路径 | 默认值 | 说明 |
|----------|--------|------|
| `/proc/sys/vm/swappiness` | 60 | Swap倾向性（0-100） |
| `/proc/sys/vm/dirty_ratio` | 20 | 脏页占比阈值（触发同步刷盘） |
| `/proc/sys/vm/dirty_background_ratio` | 10 | 后台刷盘阈值 |
| `/proc/sys/vm/overcommit_memory` | 0 | 内存超额分配策略 |
| `/proc/sys/vm/min_free_kbytes` | 依赖内存 | 最小空闲内存 |
...
# 使用perf分析缺页中断
perf stat -e faults -p [pid] sleep 10

# 查看缺页中断详情
perf record -e exceptions:page_fault_user -p [pid] sleep 10
```
