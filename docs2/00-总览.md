...
| 深入理解原理 | 总览 → 按章节顺序阅读（线程与并发 → 内存管理 → 文件存储 → I/O多路复用 → 网络通信 → 中断与信号） |
...
flowchart LR
    ...
    style P1 fill:#BBDEFB,stroke:#1565C0,color:#0D47A1
    style P2 fill:#BBDEFB,stroke:#1565C0,color:#0D47A1
    style P3 fill:#BBDEFB,stroke:#1565C0,color:#0D47A1
    style R1 fill:#C8E6C9,stroke:#388E3C,color:#1B5E20
    style R2 fill:#C8E6C9,stroke:#388E3C,color:#1B5E20
    style R3 fill:#C8E6C9,stroke:#388E3C,color:#1B5E20
    style C1 fill:#FFE0B2,stroke:#EF6C00,color:#E65100
    style C2 fill:#FFE0B2,stroke:#EF6C00,color:#E65100
    style C3 fill:#FFE0B2,stroke:#EF6C00,color:#E65100
```
...
| GC | 垃圾回收（Garbage Collection） | JVM自动内存管理机制 |
...
RocketMQ 的六大领域按功能层次划分为四层架构，从核心基础层到网络通信层逐级依赖，形成完整的消息处理链路。
...
flowchart LR
    ...
    subgraph IOLayer["I/O与存储层"]
        ...
        I1["I/O多路复用<br/>Netty Epoll<br/>ChannelPipeline<br/>Netty线程模型"]
    end
...
| 层次 | 包含领域 | 核心职责 | 关键组件 |
|------|---------|---------|---------|
| 核心基础层 | 线程与并发、中断与信号 | 提供线程生命周期管理和中断信号处理，是整个系统的执行基础 | ServiceThread、CountDownLatch2、分段锁 |
| 内存管理层 | 内存管理 | 通过mmap映射和堆外内存池优化内存访问性能 | MappedFile、transferTo、TransientStorePool |
| I/O与存储层 | 文件存储、I/O多路复用 | 实现高效的文件持久化和事件驱动I/O处理 | CommitLog、StoreCheckpoint、Netty Epoll |
| 网络通信层 | 网络通信 | 处理TCP连接管理和网络参数优化 | SO_BACKLOG、TCP_NODELAY、IdleStateHandler |

**六大领域核心概念**：

| 领域 | 核心概念 | RocketMQ应用 |
|------|---------|-------------|
| 线程与并发 | 线程状态、futex、锁机制、原子操作 | ServiceThread、CountDownLatch2、分段锁 |
| 内存管理 | 虚拟内存、mmap、零拷贝、堆外内存 | MappedFile、transferTo、TransientStorePool |
| 文件存储 | 顺序写、预分配、PageCache、检查点 | CommitLog顺序写、异步刷盘、StoreCheckpoint |
| I/O多路复用 | epoll、Reactor、事件驱动 | Netty Epoll、ChannelPipeline、Netty线程模型 |
| 网络通信 | TCP参数、三次握手、空闲检测 | SO_BACKLOG、TCP_NODELAY、IdleStateHandler |
| 中断与信号 | 睡眠状态、信号处理、SIGKILL | TASK_INTERRUPTIBLE、TASK_UNINTERRUPTIBLE |
